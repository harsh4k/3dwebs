...
export const compositeFragment = /* glsl */ `
  ...
  struct Palette { vec3 ink; vec3 accent; vec3 paper; float gamma; float mono; };

  vec3 ramp(Palette p, float t) {
    return t < 0.5 ? mix(p.ink, p.accent, t * 2.0) : mix(p.accent, p.paper, (t - 0.5) * 2.0);
  }
  ...
  void main() {
    ...
    float edge = pow(smoothstep(uFocusSize, 1.0, d), uEdgePower);
    ...
    float keep = 1.0 - card.b * uHoverClean;
    float onCard = step(0.001, card.r);
    float distance = 1.0 - scene.a;
    float dissolve = max(edge * onCard, distance * uDepthBlur);
    float recession = distance * uDitherDepth;
    float ditherLoose = pow(smoothstep(uDitherStart, 1.0, recession), uDitherPower);
    float ditherTight = pow(smoothstep(uStageDitherBegin, 1.0, dissolve), uDitherPower);
    ...
    float ditherDrive = mix(ditherLoose, ditherTight, uCoupling) * onCard;
    float softness = max(blurDrive, dim * uHoverBlur) * keep;
    float lvl = softness * uBlurStrength * 4.0;
    float fade = edge * uFadeStrength * keep * onCard;
    ...
    vec3 edgeSource = uDitherScale > 1.0 ? toDisplay(composed(cellUv(uDitherScale, push), lvl), fade) : c;
    ...
    float hoverRamp = smoothstep(uHoverDitherCutoff, 1.0, pow(card.g, uHoverDitherCurve)) * uHoverDither;
    ...
    vec3 hoverSource = uHoverDitherScale > 1.0
      ...
      : c;
    Palette hoverPalette = Palette(uHoverInk, uHoverAccent, uHoverPaper, uHoverGamma, uHoverMono);
    result = mix(
      result,
      poster(hoverPalette, hoverSource, hoverThreshold, max(uHoverDitherLevels, 2.0)),
    ...
    vec3 trailSource = uTrailScale > 1.0 ? toDisplay(composed(cellUv(uTrailScale, push), lvl), fade) : c;
    trailSource = grade(trailSource, trailMask);
    Palette trailPalette = Palette(uTrailInk, uTrailAccent, uTrailPaper, uTrailGamma, uTrailMono);
    result = mix(
      result,
    ...
    float rim = 1.0 - smoothstep(uTrailRimThickness - rimSoft, uTrailRimThickness + rimSoft, rimDist);
    result = mix(result, uTrailRimColor, rim * uTrailRim);

    float entryThreshold = bayer8(gl_FragCoord.xy / uEntryScale);
    vec3 entrySource = uEntryScale > 1.0 ? toDisplay(composed(cellUv(uEntryScale, push), lvl), fade) : c;
    Palette entryPalette = Palette(uEntryInk, uEntryAccent, uEntryPaper, uEntryGamma, uEntryMono);
    result = mix(
      result,
